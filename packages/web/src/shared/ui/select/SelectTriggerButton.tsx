  ...
  ({ value, placeholder = "Выберите...", onClear, showClearButton = false, children, className, ...props }, ref) => {
    return (
      <div className="relative">
        ...
        >
          {value ? (
            <span className="truncate flex items-center gap-2 flex-1 min-w-0 text-left font-normal [&_*]:font-normal">
              {children}
            </span>
          ) : (
            <span className="text-muted-foreground text-left font-normal">{placeholder}</span>
          )}
