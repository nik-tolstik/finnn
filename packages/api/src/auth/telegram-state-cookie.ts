export const TELEGRAM_STATE_COOKIE_NAME = "finnn_telegram_state";
...
export function createTelegramStateCookie(value: string, maxAgeSeconds: number): string {
  const { sameSite, secure } = getCookieOptions();
  const parts = [
    `${TELEGRAM_STATE_COOKIE_NAME}=${encodeURIComponent(value)}`,
    "HttpOnly",
...
export function createClearTelegramStateCookie(): string {
  const { sameSite, secure } = getCookieOptions();
  const parts = [
    `${TELEGRAM_STATE_COOKIE_NAME}=`,
    "HttpOnly",
    "Path=/auth/telegram/callback",
    "Max-Age=0",
...
export function parseTelegramStateCookie(cookieHeader: string | undefined): string | null {
  if (!cookieHeader) return null;

  for (const cookie of cookieHeader.split(";")) {
    const [name, ...rawValue] = cookie.trim().split("=");
    if (name === TELEGRAM_STATE_COOKIE_NAME) {
