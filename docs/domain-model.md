...
`DebtTransaction.description` is an optional user-provided note for ordinary debt repayments. It is stored on the debt transaction and shown in the combined transaction list and repayment details. Debt write-offs keep their description on the linked `PaymentTransaction` and do not duplicate it on the debt transaction.
